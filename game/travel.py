...
def path_to(actor: tcod.ecs.Entity, dest: Position) -> list[Position]:
    ...
    # assert dest.map is map_ (We don't do that here.)

    # Copy the walkable array.
    cost = TILES["walk_cost"][map_.components[Tiles]]

    for other in actor.registry.Q.all_of(components=[Position], tags=[IsActor], relations=[(IsIn, map_)]):
    ...
    return [Position(ij_index[1], ij_index[0]) for ij_index in path]